...
if ($_SESSION['admin_level'] == 7) {
  ...
  if ($result) {
    $output = '<div class="alert alert-success">Query executed successfully.</div>';

    // Display the results in a table
    if ($result->num_rows > 0) {
      $output .= '<style>.queryTable{text-align:center;}</style><table class="table queryTable table-bordered m-2" id="results"><tr>';
    ...
    $output = '<div class="alert alert-danger">Error executing query: ' . $mysqli->error . '</div>';
  }
} else {
